...
test('refreshQuotas checks token accounts with a concurrency limit of two', async () => {
  const configs = [
    createConfig(0, { available: true, reason: 'ok' }),
    createConfig(1, { available: true, reason: 'ok' }),
    createConfig(2, { available: true, reason: 'ok' }),
    createConfig(3, { available: true, reason: 'ok' }),
  ];
  const quotaPayload = {
    rate_limit: {
      allowed: true,
  ...
  const requestBufferedFn = () => {
    ...
    return new Promise(resolve => {
      pending.push(() => {
        ...
        resolve({
          statusCode: 200,
          bodyText: JSON.stringify(quotaPayload),
        });
      });
    });
  };
  const { manager } = createManager(configs, { requestBufferedFn });

  const refreshPromise = manager.refreshQuotas('poll');
