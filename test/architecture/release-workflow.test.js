...
test('release workflow normalizes macOS DMG names with tag version and user-facing architecture', () => {
  ...
  assert.match(workflow, /AI_Cockpit_\$\{version\}_\$\{asset_arch\}\.dmg/);
  assert.doesNotMatch(workflow, /AI_Cockpit_\$\{version\}_aarch64\.dmg/);
  assert.doesNotMatch(workflow, /Airouter_\$\{version\}/);
});
...
test('release workflow renames the Windows installer with version and architecture', () => {
  ...
  assert.match(workflow, /AI_Cockpit_\$\{version\}_x64-setup\.exe/);
  assert.doesNotMatch(workflow, /Airouter_\$\{version\}/);
