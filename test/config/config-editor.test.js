...
test('buildImportedConfigItem preserves token price_yuan from imported token config', () => {
  const imported = buildImportedConfigItem('token', {
    account_id: 'account-from-fields',
    access_token: 'access-token-from-fields',
    description: 'primary account',
    price_yuan: '20.50',
  });

  assert.equal(imported.price_yuan, '20.50');
});
...
test('buildImportedConfigItem rejects token input without required session fields', () => {
  assert.throws(() => {
    ...
    assert.equal(err instanceof ConfigEditorError, true);
    assert.match(err.message, /Token JSON/);
    return true;
  });
});
