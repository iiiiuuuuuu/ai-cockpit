...
const { formatAccountLabel } = require('./app/accounts/account-label');
const { createAccountManager } = require('./app/accounts/account-manager');
const { refreshOpenAIToken } = require('./app/accounts/openai-token-refresh');
const {
...
} = require('./app/protocols/responses-failover');
const {
    ...
    getConfigItemType
} = require('./app/config/openai-config');
const {
    ConfigEditorError,
    addConfigItem,
...
function createMissingConfigResponse(res) {
    return res.status(503).json({
        ...
        message: '当前没有可用配置，请先在 AI Cockpit App 中添加账号'
    });
}
    ...
    await listenOnPort(runtimePort);
    {
        ...
        void (async () => {
            ...
            if (apiConfigs.length === 0) {
                log('  - 当前没有配置项，请先在 AI Cockpit App 中添加账号');
            }
            log('');
            log('路由规则:');
