...
function buildImportedConfigItem(typeOrItem, maybeItem) {
    ...
    const explicitAlias = normalizeString(item.alias);
    ...
    if (!accessToken || !accountId) {
        throw new ConfigEditorError('token 模式下请提供 access_token/account_id，或直接粘贴包含 user.email、account.id、accessToken 的 Token JSON');
    }
    ...
    if (Object.prototype.hasOwnProperty.call(item, 'price_yuan')) {
        imported.price_yuan = item.price_yuan;
    }
