...
function createAccountManager(options) {
  ...
  const resolvedQuotaRefreshConcurrency = Math.max(1, Number.isFinite(Number(quotaRefreshConcurrency))
    ? Math.floor(Number(quotaRefreshConcurrency))
  ...
  async function refreshConfigsWithConcurrency(targetConfigs, reason) {
    let nextIndex = 0;
    const workerCount = Math.min(resolvedQuotaRefreshConcurrency, targetConfigs.length);
  ...
  async function refreshQuotas(reason = 'poll') {
    ...
    try {
      const monitoredConfigs = configs.filter(config => shouldUseQuotaMonitoring(config.type));
      await refreshConfigsWithConcurrency(monitoredConfigs, reason);
